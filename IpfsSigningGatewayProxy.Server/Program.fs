...
// type IIpfsHttpGateway =
//     abstract GetCid: cid: Cid * ?subdomain: bool * ?path: string * ?``params``: string * headers: HttpRequestHeaders -> Task<HttpResponseMessage>
        ...
        | Equals HttpMethods.Get ->
            ...
            | Ok (Some signingAddress) ->
                ...
                | Error errorResult ->
                    ...
                    return! ctx.Response.CompleteAsync()
                | Ok varsigStr ->
                    match verifyVarsigSignature signingAddress publicKey varsigStr with
                    | Error err ->
